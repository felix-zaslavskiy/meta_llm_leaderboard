...
# Load the data
df = pd.read_csv('../temp_data/bigcode_data.csv')

# Find the best model within each size_type
best_models = df.loc[df.groupby("size_type")["Win Rate"].idxmax()]
...
for i, row in best_models.iterrows():
    ...
    if len(model_name) > row['Win Rate'] * 1.5:
